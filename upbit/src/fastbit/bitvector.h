...
public:
    ...
    // combine getBit and setBit into one function
    void flipBit(const word_t i);
    ...
    std::vector<std::pair<int, int>> fencePointers;
    ...
    void decode(std::vector<uint64_t> &rowids);

    void decode(std::vector<uint64_t> &rowids, ibis::bitvector &ub);
...
private:
    ...
    inline void decode_runs(run &it, word_t &nw, std::vector<uint64_t> &rids); // copy nw words
    inline void decode_runsn(run &it, word_t &nw, std::vector<uint64_t> &rids); // copy nw words and negate

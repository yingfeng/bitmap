...
void ibis::bitvector::decode(std::vector<uint64_t> &rids) {
    auto begin = m_vec.begin();
    uint64_t jnd = 0;

    if (this->isCompressed()) {
        while (begin != m_vec.end()) {
            if (*begin > HEADER0) {
                // a fill
                const word_t cnt = ((*begin) & MAXCNT) * MAXBITS;
                if (*begin >= HEADER1) {
                    // 1-fill
                ...
                while (curt != 0) {
                    word_t t = curt & -curt;
                    int r = __builtin_ctzl(curt);
                    rids.push_back(jnd + 30 - r);
                    curt ^= t;
                }
                jnd += MAXBITS;
            }
    ...
    // active word
    if (active.nbits > 0) {
        word_t curt = active.val;
        while (curt != 0) {
            word_t t = curt & -curt;
            int r = __builtin_ctzl(curt);
            rids.push_back(jnd + r);
            curt ^= t;
        }
    }
}

void ibis::bitvector::decode(std::vector<uint64_t> &rids, ibis::bitvector &ub) {
    int bi_idx_pos = 0;
    uint64_t jnd = 0;
    run x, y;
    x.it = m_vec.begin();
    ...
    while (x.it < m_vec.end()) {    // go through all words in m_vec
        ...
        if (x.isFill != 0) { // x points to a fill
            ...
            if (y.isFill != 0 && y.nWords >= x.nWords) {
                if (y.fillBit == 0) {
                    // copy_runs(x, y.nWords);
                    if (x.fillBit != 0) {
                        // 1-fill
                        for (uint32_t i = 0; i < x.nWords * MAXBITS; ++i) {
                            rids.push_back(jnd + i);
                        }
                    }
                    jnd += x.nWords * MAXBITS;
                    y.nWords -= x.nWords;
                    ++x.it;
                    x.nWords = 0;
                    while (y.nWords > 0) {
                        x.decode();
                        if (y.nWords >= x.nWords) {
                            if (x.isFill != 0) {
                                // a fill
                                if (x.fillBit != 0) {
                                    // 1-fill
                ...
                if (y.isFill != 0) {
                    if (y.fillBit != 0) {
                        // 1-fill
                        for (uint32_t i = 0; i < y.nWords * MAXBITS; ++i) {
                            rids.push_back(jnd + i);
                        }
                    }
                } else {
...
void ibis::bitvector::buildFencePointers() {
    int numWords = 0;
    run it;
    it.it = m_vec.begin();
    while (it.it < m_vec.end()) {
        it.decode();
        numWords += it.nWords;
        if (numWords > INDEX_WORDS) {
            fencePointers.push_back({it.vecPos, it.wordPos});
            numWords = 0;
        }
        ++it.it;
    }
...
int ibis::bitvector::getBitWithIndex(const ibis::bitvector::word_t ind) const {
    ...
    for (int i = 0; i < fencePointers.size(); ++i) {
        indPos = i - 1;
        if (fencePointers[i].second >= vecPos) {
            break;
        }
    }
    if (indPos >= 0) {
        jnd -= fencePointers[indPos].second * MAXBITS;
        it += fencePointers[indPos].first + 1;
    }
...
void ibis::bitvector::setBit(const ibis::bitvector::word_t ind, int val) {
    ...
    if (m_vec.size() * MAXBITS == nbits) { // uncompressed
        ...
        if (kEnableFencePointer && fencePointers.size() > 0) {
            int indPos = -1;
            int vecPos = ind / MAXBITS;
            for (int i = 0; i < fencePointers.size(); ++i) {
                indPos = i - 1;
            ...
            if (indPos >= 0) {
                ind0 -= fencePointers[indPos].second * MAXBITS;
                it += fencePointers[indPos].first + 1;
            }
        }
...
void ibis::bitvector::flipBit(const ibis::bitvector::word_t ind) {
    m_vec.nosharing(); // make sure the array is not shared
    int val = 1;
    if (ind >= size()) {
        word_t diff = ind - size() + 1;
        ...
        word_t u = active.val;
        word_t w = (word_t) (1 << (active.nbits - (ind - nbits) - 1));
        val = active.val & w;
        if (val == 0) {
            active.val |= w;
        } else {
    ...
    if (m_vec.size() * MAXBITS == nbits) { // uncompressed
        const word_t i = ind / MAXBITS;
        const word_t u = m_vec[i];
        const word_t w = (const word_t) (1 << (SECONDBIT - (ind % MAXBITS)));
        val = m_vec[i] & w;
        if (val == 0)
            m_vec[i] |= w;
        else
        ...
        while (x.it < m_vec.end()) { // go through all words in m_vec
            ...
            if (x.isFill != 0) {        // x points to a fill
                ...
                if (y.isFill != 0) {
                    ...
                    if (kEnableFencePointer)
                        res.copy_runs(y, x.nWords, rhs.fencePointers, bi_idx_pos);
                    else
...
void ibis::bitvector::xor_c2(const ibis::bitvector &rhs,
    ...
    x.it = m_vec.begin();
    y.it = rhs.m_vec.begin();
    res.m_vec.reserve(rhs.m_vec.size() + this->m_vec.size() + 10);
